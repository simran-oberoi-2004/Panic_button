...
app.use(cookieParser());// modules import
...
const uri = `mongodb+srv://${username}:${password}@cluster0.njjfdf5.mongodb.net/${databaseName}?retryWrites=true&w=majority`;
mongoose
  .connect(uri)
  .then(() => {
...
//routes of my apps (pages visibility)
app.get("/", (req, res) => {
...
app.post("/signin", async (req, res) => {
  ...
  try {
    ...
    console.log("Cookie Set:", user._id.toString()); 
...
app.post('/sendLocation', async (req, res) => {
  const { message,latitude, longitude, userId } = req.body; 
  ...
  try {
    ...
    // Send Twilio message with Google Maps link as the body to the user's mobile number (twilio documentation)
